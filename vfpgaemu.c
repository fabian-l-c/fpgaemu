/*  fpgaemu.c - Graphical interface for the FPGA board emulator.
    Copyright (C) 2022  Fabian L. Cabrera
...
int *inmemloc = (int*)(&inmem[0]);
int *outmemloc1 = (int*)(&outmem[0]);
int *outmemloc2 = (int*)(&outmem[4]);
int *outmemloc3 = (int*)(&outmem[8]);
int *ckmemloc = (int*)(&ckmem[0]);

static pthread_t thread_id; 
vpiHandle argh,argh2;
struct t_vpi_value argval,argval2;
...
void update_leds(void)
{
   ...
   int i,j;
   argval.value.integer = *ckmemloc;
   vpi_put_value(argh, &argval, NULL, vpiForceFlag);
...
void clear_input(GtkWidget *widget, gpointer   data)
{
   ...
   argval2.value.integer = *inmemloc;
   vpi_put_value(argh2, &argval2, NULL, vpiForceFlag);
}
...
static int inp_compiletf(char*user_data)
{
      return 0;
}

static int inp_calltf(char*user_data)
{
...
void inp_register()
{
      ...
      tf_data.tfname    = "$vinp";
      tf_data.calltf    = inp_calltf;
      tf_data.compiletf = inp_compiletf;
      tf_data.sizetf    = 0;
      tf_data.user_data = 0;
      vpi_register_systf(&tf_data);
}

static int outp_compiletf(char*user_data)
{
      return 0;
}

static int outp_calltf(char*user_data)
{
vpiHandle systfref, args_iter;
vpiHandle argh3,argh4,argh5;
struct t_vpi_value argval3,argval4,argval5;
...
*outmemloc1 = argval3.value.integer;

argh4 = vpi_scan(args_iter);
argval4.format = vpiIntVal;
vpi_get_value(argh4, &argval4);
*outmemloc2 = argval4.value.integer;

argh5 = vpi_scan(args_iter);
argval5.format = vpiIntVal;
vpi_get_value(argh5, &argval5);
*outmemloc3 = argval5.value.integer;
...
void outp_register()
{
      ...
      tf_data.type      = vpiSysTask;
      tf_data.tfname    = "$voutp";
      tf_data.calltf    = outp_calltf;
      tf_data.compiletf = outp_compiletf;
      tf_data.sizetf    = 0;
      tf_data.user_data = 0;
      vpi_register_systf(&tf_data);
}
